package mapony.inferencia.util.pattern.reservoirsampler;
 ...
 * <p>
 * Several optimizations can be done. Especially, one can avoid rolling the dice as many times as the size of the population with an involved trick.
...
public class ReservoirSampler<T> {
	private final String errNegativeNumSamples = "numSamples should be positive";
	 ...
	 * 
	 * @param numSamples
	 *            Maximum number of samples to retain in the reservoir. Must be non-negative.
	 * @param random
	 *            Instance of the random number generator to use for sampling
	 */
	public ReservoirSampler(int numSamples, Random random) {
		Preconditions.checkArgument(numSamples > 0, errNegativeNumSamples);
	 ...
	 * 
	 * @param item
	 *            The item to sample - may not be null.
	 */
