 ...
 *         <p>
 *         Mapper para el Job <code>GroupNearByCity</code>.
 *         <p>
 *         Recibe una linea de texto, y la parsea a un CustomWritable Emite como clave el GeoHash calculado con la precision que se ha definido para
 *         el job, y el RawData generado
 *         <p>
 *         Recuperamos de memoria un HashMap que contiene unicamente los datos de Madrid, Berlin, Roma y Londres. Se compara el geohash de cada
 *         registro procesado con el de estas ciudades y solo se emite resultado si coinciden.
 */
public class GroupNearByCityMap extends Mapper<LongWritable, Text, Text, RawData> {

	private static Logger logger;
	private HashMap<String, GeoHashBean> selectedCities;
	...
	protected void map(LongWritable offset, Text line, Context context) throws IOException, InterruptedException {
		...
		try {
			if (ComplexValidation.isRecordUseful(dato)) {
				...
				if (selectedCities.containsKey(calculatedGeoHash)) {
					final GeoHashBean ghb = selectedCities.get(calculatedGeoHash);
					// Creamos el Writable
					RawData rdBean = new RawData(dato, ghb);
	...
	public void setup(Context context) {
		logger = LoggerFactory.getLogger(GroupNearByCityMap.class);
		geoHashPrecission = Integer.parseInt(context.getConfiguration().get(PropertiesCte.precision));

		try {
			if (null == selectedCities) {
				selectedCities = GeoHashCiudad.loadSelectedCities(geoHashPrecission);
			}
		} catch (Exception e) {
			logger.error(e.getMessage());
		}
